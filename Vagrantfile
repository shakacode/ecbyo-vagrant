...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
    ...
    config.vm.synced_folder "./", "/var/www", owner: "ubuntu", group: "www-data", mount_options: ["dmode=777,fmode=777"]
    ...
    config.vm.provider "virtualbox" do |vb|
        ...
        vb.customize ["modifyvm", :id, "--name", "ecbyo"]
    ...
    config.hostmanager.aliases =  ["ecbyo.local"]
    ...
    $script = <<SCRIPT
        ...
        ansible-galaxy install -r /var/www/requirements.yml
SCRIPT
    ...
    config.vm.provision "shell" do |sh|
        sh.inline = "ansible-playbook /var/www/provisioning/main.yml -i 'ubuntu,' --connection=local"
    end
end
